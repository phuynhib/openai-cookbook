...
def search_redis(
    ...
    return_fields: list = ["title", "article_id", "text", "vector_score"],
    hybrid_fields = "*",
    k: int = 20,
...
def time_queries(iterations: int = 10):
    ...
    for i in range(iterations):
        results_flat = search_redis(redis_client, query, k=10, print_results=False, vector_field='content_vector')
    t0 = (time.time() - t0) / iterations
    results_flat = search_redis(redis_client, query, k=10, print_results=True, vector_field='content_vector')
    print(f"Flat index query time: {round(t0, 3)} seconds\n")
    time.sleep(1)
    print(" ----- HNSW Index ------ ")
    t1 = time.time()
    for i in range(iterations):
        results_hnsw = search_redis(redis_client, query, index_name=HNSW_INDEX_NAME, k=10, print_results=False, vector_field='content_vector')
    t1 = (time.time() - t1) / iterations
    results_hnsw = search_redis(redis_client, query, index_name=HNSW_INDEX_NAME, k=10, print_results=True, vector_field='content_vector')
